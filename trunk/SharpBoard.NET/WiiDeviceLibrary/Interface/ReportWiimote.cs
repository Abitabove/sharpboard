...
namespace WiiDeviceLibrary
{
    ...
    public class ReportWiimote : ReportDevice, IWiimote
    {
        ...
        public CameraSensitivity IRSensitivity
        {
            get { return _cameraSensitivity; }
            set 
            { 
                _cameraSensitivity = value;
                SetIRMode(irMode, _cameraSensitivity);
            } 
        }
        ...
        public bool IsIRSensorEnabled
        {
            get { return _IsIRSensorEnabled; }
        }
        ...
        // Change by delfo
        public void SetReportingMode(ReportingMode reportMode, CameraSensitivity irMode)
        {
            SetIRMode(GetIrMode((InputReport)reportMode),irMode);
            base.SetReportingMode(reportMode);
        }
        public override void SetReportingMode(ReportingMode reportMode)
        {
            SetReportingMode(reportMode, CameraSensitivity.Max);
        }
        // End change
        ...
        #region Set Methods
        protected void SetSpeaker(bool enabled, byte frequency, byte volume)
        {
        ...
        public void SetIRMode(IRMode irMode, CameraSensitivity sensitivity)
        {
            ...
            if (enabled)
            {
                ...
                switch (sensitivity)
                {
                    case CameraSensitivity.Level1:
                        WriteMemory(0xb00000, new byte[] { 0x02, 0x00, 0x00, 0x71, 0x01, 0x00, 0x64, 0x00, 0xfe });
                        WriteMemory(0xb0001a, new byte[] { 0xfd, 0x05 });
                        break;
                    case CameraSensitivity.Level2:
                        WriteMemory(0xb00000, new byte[] { 0x02, 0x00, 0x00, 0x71, 0x01, 0x00, 0x96, 0x00, 0xb4 });
                        WriteMemory(0xb0001a, new byte[] { 0xb3, 0x04 });
                        break;
                    case CameraSensitivity.Level3:
                        WriteMemory(0xb00000, new byte[] { 0x02, 0x00, 0x00, 0x71, 0x01, 0x00, 0xaa, 0x00, 0x64 });
                        WriteMemory(0xb0001a, new byte[] { 0x63, 0x03 });
                        break;
                    case CameraSensitivity.Level4:
                        WriteMemory(0xb00000, new byte[] { 0x02, 0x00, 0x00, 0x71, 0x01, 0x00, 0xc8, 0x00, 0x36 });
                        WriteMemory(0xb0001a, new byte[] { 0x35, 0x03 });
                        break;
                    case CameraSensitivity.Level5:
                        WriteMemory(0xb00000, new byte[] { 0x07, 0x00, 0x00, 0x71, 0x01, 0x00, 0x72, 0x00, 0x20 });
                        WriteMemory(0xb0001a, new byte[] { 0x1f, 0x03 });
                        break;
                    case CameraSensitivity.Max:
                        WriteMemory(0x04b00000, new byte[] { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x90, 0x00, 0x41 });
                        WriteMemory(0x04b0001a, new byte[] { 0x40, 0x00 });
                        break;
                    default:
                        break;
                }
        ...
        {
            SetIRMode(irMode, CameraSensitivity.Max);
        }
        ...
        protected void ParseStatus(byte[] buff)
        {
            ...
            _IsIRSensorEnabled = (buff[3] & 0x08) != 0;
